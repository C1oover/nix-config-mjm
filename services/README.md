...
For instance, `services/foo/default.nix` will define a `cloover.foo.enable` option and only configure the service if it is set to `true`.
...
I have some special options under [`cloover.state`](../modules/nixos/state.nix) that allow for declaring files and directories that needs persisting without needing to know the root directory of the persistent storage.
...
My services can add their data directories to `cloover.state.directories` when the service is enabled, and these directories will end up persisted to whatever `cloover.state.persistDir` is configured as for that machine.
