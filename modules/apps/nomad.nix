...
let
  ...
  serviceCheckType = with lib; {
    options = {
      ...
      interval = mkOption { type = types.int; };
  ...
  serviceType = with lib; {
    options = {
      ...
      port = mkOption { type = types.either types.port types.str; };
      task = mkOption {
        type = types.nullOr types.str;
        default = null;
      };
  ...
  config =
    let
      mkTemplateConfig =
        ...
        {
          EmbeddedTmpl =
            if tmpl.source != null then
              builtins.readFile tmpl.source
            else if tmpl.text != null then
              tmpl.text
            else
              null;
      ...
      mkDockerTaskOpts = task: {
        ...
        Config =
          ...
          // (lib.optionalAttrs (task.ports != null) { inherit (task) ports; })
      ...
      mkServiceCheckConfig =
        svcIdx: svc: checkIdx: check:
        let
          commonOpts = {
        ...
        in
        if check.http.path != null then
          {
            Type = "http";
            Path = check.http.path;
          }
          // (lib.optionalAttrs (check.http.port != null) { PortLabel = check.http.port; })
          // (lib.optionalAttrs (check.http.headers != null) { Header = check.http.headers; })
          // (lib.optionalAttrs svc.connect.enable {
            Expose = true;
            PortLabel = "health_${toString svcIdx}_${toString checkIdx}";
          })
          // commonOpts
        else if check.script != null then
          {
            Type = "script";
      ...
      mkServiceConfig =
        i: svc:
        {
          ...
          Meta =
            (
              lib.optionalAttrs svc.metrics.enable { metrics_path = svc.metrics.path; }
        ...
        [
          {
            ...
            DynamicPorts =
              (map (port: { Label = port.label; } // (lib.optionalAttrs (port.to != null) { To = port.to; }))
                dynamicPorts
              )
              ++ connectPorts;
            ReservedPorts =
              map
                ...
                reservedPorts;
            Mode = if builtins.any (s: s.connect.enable) tg.services then "bridge" else "host";
          }
        ];
